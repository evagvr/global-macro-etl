...
class PipelineTransformer(BaseTransformer):
    def __init__(self, df_macro: pd.DataFrame, df_stocks: pd.DataFrame, df_forex: pd.DataFrame, logger: logging.Logger):
    ...
    def merge_all(self) -> pd.DataFrame:
        ...
        df_merged = pd.merge_asof(left=self.df_merged, right=self.df_macro, on="date", direction="backward")
